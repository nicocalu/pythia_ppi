...
def read_pdb_to_protbb(pdb_file: str, return_chain_dict=False):
    cas = []
    cbs = []
    cs = []
    os = []
    ...
    if return_chain_dict:
        return ProtBB(cas, cbs, cs, os, ns, seqs, resseqs, chains, bb_ang, sasa), chain_dict
    else:
        return ProtBB(cas, cbs, cs, os, ns, seqs, resseqs, chains, bb_ang, sasa)

def get_neighbor(protbb, neighbor: int = 32, noise_level=0.0, train=False, mask=True):
    # L, 1, 3
    device = "cuda"
    L = len(protbb.ca)
    if L < neighbor:
    ...
    protbb.ca = protbb.ca.to(device)
    protbb.cb = protbb.cb.to(device)
    protbb.c = protbb.c.to(device)
    protbb.o = protbb.o.to(device)
    protbb.n = protbb.n.to(device)
    protbb.seq = protbb.seq.to(device)
    protbb.resseq = protbb.resseq.to(device)
    protbb.chain_num = protbb.chain_num.to(device)
    protbb.bb_ang = protbb.bb_ang.to(device)
